...
ui_sidebar <- function(id) {
  ...
  ui <- bslib::sidebar(
    ...
    # Left-censoring warning ----------------------------------------------
    
    # Shown only when more than 80% of the uploaded measurements
    # (either overall or within the selected category) are left-censored.
    bslib::card(
      id    = ns("censoring_warning"),
      class = "border-warning bg-warning-subtle",
      fill  = FALSE,
      
      bslib::card_body(
        class = "p-2",
        shiny::textOutput(ns("censoring_warning_text"), tags$small)
      )
    ) |>
      shinyjs::hidden(),
...
server_sidebar <- function(id, lang, panel_active) {
  ...
  server <- \(input, output, session) {
    ...
    censoring_warning_text <- shiny::reactive({
      translate(lang = lang(), "
                With such a high proportion of censored values, your data
                contains very little information. As a result, the statistical
                inference relies increasingly on the prior assumptions and may
                not be robust to changes in them. Expostats does not currently
                allow prior customization; until it does, we recommend using
                IHSTAT_Bayes to assess how sensitive your conclusions are to
                different prior choices.
            ")
    }) |>
      shiny::bindCache(lang())
    
    output$censoring_warning_text <- shiny::renderText({
      censoring_warning_text()
    }) |>
      shiny::bindCache(lang())
    
    # Compute the proportion of left-censored measurements in the relevant
    # slice of the uploaded data:
    #   - the chosen category (if data_chosen_variable and
    #     data_chosen_category are both set), or
    #   - the whole dataset otherwise.
    # A measurement is treated as left-censored when its (character)
    # representation begins with "<" (optionally preceded by whitespace).
    left_censored_fraction <- shiny::reactive({
      d <- data()
      ...
      measurements <- as.character(d[[measurements_column_index]])
      
      chosen_variable <- input$data_chosen_variable
      chosen_category <- input$data_chosen_category
      
      if (
        !is.null(chosen_variable) && nzchar(chosen_variable) &&
        !is.null(chosen_category) && nzchar(chosen_category) &&
        chosen_variable %in% names(d)
      ) {
        keep <- d[[chosen_variable]] == chosen_category
        keep[is.na(keep)] <- FALSE
        measurements <- measurements[keep]
      }
      
      measurements <- measurements[!is.na(measurements) &
                                     nzchar(trimws(measurements))]
      
      if (!length(measurements)) {
        return(0)
      }
      
      sum(grepl("^\\s*<", measurements)) / length(measurements)
    })
    
    # Toggle the warning banner based on the >80% threshold.
    shiny::observe({
      shinyjs::toggle(
        "censoring_warning",
        condition = left_censored_fraction() > 0.80
      )
    })
